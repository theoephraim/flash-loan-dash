    ...
    _.each(allProps, (options, key) => {
      ...
      if (visible) {
        if (options === 'json' || options.type === 'json') {
          json[key] = this[key];
          if (options.hideKeys) json[key] = _.omit(this[key], options.hideKeys);
        } else if (options === 'wei' || options.type === 'wei') {
          json[key] = this[key] ? this[key].toFixed() : null; // handle bignumbers
        } else {
          json[key] = this[key];
        }
      }
